...
func (m *metricValue) setGauge(ident, metric string, val interface{}, tags ...string) {
	m.Endpoint = ident
	m.Metric = metric
	m.ValueUntyped = val
	m.Step = int64(config.Duration)
	size := len(tags)
	if size > 0 {
...
func (m *metricValue) CheckValidity(now int64) (err error) {
	...
	if m.CounterType != gauge && m.CounterType != counter && m.CounterType != derive {
	...
	if len(m.Tags) > 512 {
		err = fmt.Errorf("len(m.Tags) is too large,%d", len(m.Tags))
		return
	}
